BUCKET="s3://ai2-prior-gpv/public"

# set to the location where you want to store downloaded data
GPV_DATA=$1

# download and extract coco and coco-sce splits
mkdir -p $GPV_DATA
mkdir $GPV_DATA/learning_phase_data
aws s3 cp $BUCKET/coco_original_and_sce_splits.zip $GPV_DATA/learning_phase_data/
unzip $GPV_DATA/learning_phase_data/coco_original_and_sce_splits.zip -d $GPV_DATA/learning_phase_data/

# download detr pretrained on coco and coco-sce
mkdir $GPV_DATA/detr
aws s3 cp $BUCKET/detr/detr_coco.pth $GPV_DATA/detr/
aws s3 cp $BUCKET/detr/detr_coco_sce.pth $GPV_DATA/detr/

# Download coco images
python -m data.coco.download download_coco_images_only=True output_dir=$GPV_DATA